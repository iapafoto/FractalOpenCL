...
float3 renderScene(float3 ro, float3 rd, const Context* ctx, float* dist) {
 ...
 // lighting        
	...
	float3 lin = (float3)(.5f) + 
            ...
            + 10.f*spe*(float3)(1.f,0.9f,0.7f)*dif
	...
	col = col*lin;
// rim
        col *= 1.f + 2.f*pow(clamp(1.f+dot(rd,nor),0.f,1.f),1.f)*(float3)(.7f,.8f,1.f);
