...
//obj = objLight;
					...
					if (res.z<.5f || res.x>=MAX_DIST_RAYMARCHING) {
						
					obj = objLight; 
//if (res.z>.4f) obj.emission *= .25f;
}else
					{
						obj = obj0;
						obj.colour =  1.f*(0.65f + 0.5f*cos( 6.2831f*res.y + (float3)(0.f,1.f,2.f) ));
					} 
				}
...
// -------------------------------------
			...
			//	}
                ...
                else if (type == SPEC)            // Ideal SPECULAR reflection
				{
					...
					rd = coneDirection(reflect(rd,n), .0125f, rng.x);
					continue;
				}
...
__kernel void render(float3 ro, float3 ww, float3 uu, const float4 sliderMins, const float4 sliderMaxs, 
  ...
  //  ta.y -= 3.f;
   ...
   // col = pow(col, .43f);
    ...
    float4 out = (float4)(col,1.f); //(float4)(clamp(col,(float3)(0.f), (float3)(1.f)),1.f);
