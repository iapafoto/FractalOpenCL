...
#define DE_RAY pseudoKleinianSliderSymetrique
...
inline float3 mandelBulb(float3 p, const Context* ctx) {
	p.xyz = p.xzy;
	float3 z = p;
	...
	for(int i = 0; i < 7; ++i) {
		...
		dr = fpow(r, power - 1.f) * dr * power + 1.f;
...
float pseudoKleinianSliderSymetrique(float3 p, const Context* ctx) {//knighty's pseudo kleinian
    ...
    return .75f*max(1.f-ctx->maxs.w/length(p), max(p.x, max(p.y, p.z)))/scale;
}
