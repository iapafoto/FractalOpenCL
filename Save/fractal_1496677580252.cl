...
__constant float4 sphLight = (float4)(1.f,-2.f,2.f,1.5f);
...
float3 renderScene( float3 ro, float3 rd, const Context* ctx, float* dist, float iFrame) {
		...
		float2 r, rng = (float2)(hash13(ro+100.f*hash1(iFrame)), hash13(235.24f+ro+iFrame));
		...
		Material objLight;
		objLight.emission = (float3)(1.0f,1.0f,1.0f)*3.f;
...
//obj = objLight;
					...
					obj = objLight; 
//if (res.z>.4f) obj.emission *= .025f;
}else
					{
						obj = obj0;
						obj.colour =  5.f*(0.5f + 0.5f*cos( 6.2831f*res.y + (float3)(0.f,1.f,2.f) ));
					} 
				}
