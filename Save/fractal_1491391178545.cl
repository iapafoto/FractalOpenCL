...
//    #define BACK_COLOR (float3)(.08f, .16f, .34f) 
    #define BACK_COLOR .1f*(float3)(.8f, .65f, .54f) 
#endif

#define FOCUSBLUR .005f

#define PRECISION_FACTOR 2e-4
#define MIN_DIST_RAYMARCHING .01f
#define MAX_DIST_RAYMARCHING 10.f

#define MIN_DIST_SHADOW 5.f*PRECISION_FACTOR
#define MAX_DIST_SHADOW 3.f
#define PRECISION_FACTOR_SHADOW 2e-4

#define MIN_DIST_AO 5.f*PRECISION_FACTOR
#define MAX_DIST_AO .1f
#define PRECISION_FACTOR_AO 2e-4
...
//----------------------------------------------------------------------------------------
// iq: https://www.shadertoy.com/view/MsfGRr

inline float juliaQuaternion( const float3 p, const Context* ctx )
{
    float4 z = (float4)(p,0.f);
    float md2 = 1.f;
    float mz2 = dot(z,z);
    ...
    for( int i=0; i<11; i++ )
    {
        ...
        z = (float4)( z.x*z.x-dot(z.yzw,z.yzw),
                  2.f*z.x*z.yzw ) + ctx->mins;

        mz2 = dot(z,z);
        if (mz2>4.f) break;
    }
    return .25f*sqrt(mz2/md2)*log(mz2);
}

inline float3 juliaQuaternionColor( const float3 p, const Context* ctx ) {

    float4 z = (float4)(p,0.f);
    float md2 = 1.f;
    float mz2 = dot(z,z);

    float4 trap = (float4)(fabs(z.xyz), dot(z,z));
    ...
    for( int i=0; i<11; i++ )
    {
        ...
        trap = min( trap, (float4)(fabs(z.xyz),dot(z,z)) );

        mz2 = dot(z,z);
        if (mz2>4.f) break;
    }

    return (float3)(.25f*sqrt(mz2/md2)*log(mz2), trap.x, trap.y);
}
...
float pseudoKleinianSlider(float3 p, const Context* ctx) {//knighty's pseudo kleinian
    ...
    float rxy=length(p);
    return .25f*max(rxy-ctx->maxs.w, fabs(rxy*p.y*p.x*p.z) / length(p))/scale;
}
...
float3 pseudoKleinianSliderColor(float3 p, const Context* ctx) {//knighty's pseudo kleinian
    ...
    return (float3)(0.f, 1.f-2.f*orb, orb);
}
...
float3 pal( const float t,  const float3 a,  const float3 b,  const float3 c,  const float3 d ) {
    return a + b*cos( 6.28318f*(c*t+d) );
}
...
//	col = 0.5f + 0.5f*cos( 6.2831f*res.y + (float3)(0.f,1.f,2.f) ); 
 ...
 // lighting        
	...
	col = col*lin;
// rim
        col *= 1.f + pow(clamp(1.f+dot(rd,nor),0.f,1.f),1.f)*(float3)(1.f);
