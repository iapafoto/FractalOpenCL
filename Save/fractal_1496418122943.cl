...
inline float3 hash3(float* seed) {
    float3 fractptr;
...
float2 intersect( float3 ro, float3 rd, const Context* ctx,  float3* normal, float4 lightSphere) {
	float2 res = (float2)( 1e5, -1.f );
 	float d = rayIntersect(ro, rd, ctx, PRECISION_FACTOR, MIN_DIST_RAYMARCHING, MAX_DIST_RAYMARCHING);
	if (d<res.x) res = (float2)( d, 1.f );

float t;
	
    t = iSphere( ro, rd, (float4)( 1.5,1.0, 2.7, 1.0) ); if( t>eps && t<res.x ) { res = (float2)( t, 1.f ); *normal = nSphere( ro+t*rd, (float4)( 1.5,1.0, 2.7,1.0) ); }
    t = iSphere( ro, rd, (float4)( 4.0,1.0, 4.0, 1.0) ); if( t>eps && t<res.x ) { res = (float2)( t, 6.f ); *normal = nSphere( ro+t*rd, (float4)( 4.0f,1.0f, 4.0f,1.0f) ); }
...
float3 traceEyePath( float3 ro, float3 rd, Context* ctx, bool directLightSampling, float* seed ) {
    ...
    float4 lightSphere = (float4)(1.f,1.f,1.f,4.f);
