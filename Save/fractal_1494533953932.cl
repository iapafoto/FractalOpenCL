...
float pseudoKleinianSlider(float3 p, const Context* ctx) {//knighty's pseudo kleinian
    float d1=9999.f,k,r2, scale=1.f, orb = 1.f;
    for(int i=0;i<6;i++) {
...
d1 = min(d1,sdSponge(p * ctx->maxs.w)/(ctx->maxs.w*scale));
...
//p /= scale;
//p *=  ctx->maxs.w;
//	float d1 = sdSponge(p)/ctx->maxs.w;
...
float pseudoKleinianSlider0(float3 p, const Context* ctx) {//knighty's pseudo kleinian
...
float3 pseudoKleinianSliderColor(float3 p, const Context* ctx) {//knighty's pseudo kleinian
    ...
    return (float3)(0.f, .15f+sqrt(orb), orb);
}
