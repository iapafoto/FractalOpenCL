...
#define LIGHT_VEC normalize((float3)(.2,.7, 1.) )
...
__constant float4 sphLight = (float4)(1.f,-2.f,2.f,2.5f);
...
float3 fractptr3;
float fractptr;
    p3 = fract(p3 * HASHSCALE1, &fractptr3);
    p3 += dot(p3, p3.yzx + 19.19f);
    return fract((p3.x + p3.y) * p3.z, &fractptr);
}
...
inline float2 hash2(const float seed) {
    float2 fractptr;
    return fract(sin((float2)(seed*43758.5453123f,(seed*123.027f+.3f)*22578.1459123f)), &fractptr);
}
...
float3 pseudoKleinianSliderColor(float3 p, const Context* ctx) {//knighty's pseudo kleinian
    ...
    return (float3)(0.f, .25f+.25f*sqrt(orb), orb);
}
...
float3 renderScene( float3 ro, float3 rd, const Context* ctx, float* dist, float iFrame) {
		...
		float2 r, rng = (float2)(iFrame, hash1(iFrame));

		float3 finalCol = (float3)(0.0f,0.0f,0.0f);
        float3 fCum = (float3)(1.0f,1.0f,1.0f);

		Material obj, obj0;
		obj0.emission = (float3)(0.1f,0.1f,0.1f)*0.0f;
		obj0.colour = (float3)(1.f,.5f,.5f);
		obj0.type = SPEC;

		Material objLight;
		objLight.emission = (float3)(1.0f,1.0f,1.0f)*50.f;
...
float3 pt = ro + rd * t;
if (length(pt.xy)<1.f/*|| pt.z <.01f*/) {
obj = obj0; 
obj.colour = 10.f; //(0.5f + 0.5f*cos( 6.2831f*res.y + (float3)(0.f,1.f,2.f) ));
obj.emission = obj.colour*100.f; //(float3)(1.0f,1.0f,1.0f)*50.f;
} else
					if (res.z<.5f || res.x>=MAX_DIST_RAYMARCHING || length(pt.xy)<1.f) {
						
					obj = objLight; 
if (res.z>.4f) obj.emission *= .0f;
}else
					{
						obj = obj0;
...
// -------------------------------------
                ...
                else if (type == SPEC)            // Ideal SPECULAR reflection
				{
					...
					rd = coneDirection(reflect(rd,n), .01f, rng.x);
					continue;
				}
...
__kernel void render(float3 ro, float3 ww, float3 uu, const float4 sliderMins, const float4 sliderMaxs, 
  ...
  //  ta.y -= 3.f;
    ...
    const float frameId = hash13(123456.f*q.xyx);
