...
float3 worldApplyLighting( float3 pos, float3 nor, float seed )
{
     float3 dcol = (float3)(0.0f);

     // sample sun
     {
     float3  point = 1000.f*sunDirection + 50.f*diskPoint(nor);
     float3  liray = normalize( point - pos );
     float ndl =  max(0.f, dot(liray, nor));
     dcol += ndl * sunColor * worldShadow( pos, liray );
     }

     // sample sky
     {
     float3  point = 1000.f*cosineDirection(nor, seed);
     float3  liray = normalize( point - pos );
