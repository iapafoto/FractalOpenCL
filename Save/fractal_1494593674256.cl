...
#define MIN_DIST_SHADOW 3.f*PRECISION_FACTOR
#define MAX_DIST_SHADOW 50.f
#define PRECISION_FACTOR_SHADOW 1e-3

#define MIN_DIST_AO 3.f*PRECISION_FACTOR
...
#define PALETTE_8(x) mix((float3)(0.4f,0.8f,0.1f), (float3)(.9f,.9f,.7f), .5f +.5f*sin(6.1427f*fabs(x))) ;
...
float kaliFractal(float3 p, const Context* ctx) {
    ...
    for (int i=0; i<8/*NB_ITERATIONS*/; i++) {
        p = fabs(p);
        p = p*Scale + Julia;
        p = mvmul3(ctx->rot, p);
        k *= Scale;
...
return sdSponge(p*.1f)/(k*.1f);
  //  return (length(p)-0.25f)/k;
...
//        float3 orbitTrap = (float3)(1000.f,1000.f,1000.f);
	...
	return (float3)(1.f, m, 1.f/*p.z - 0.35f*p.x*/);
	//return orbitTrap;
}
