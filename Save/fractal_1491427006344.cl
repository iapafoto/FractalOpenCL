...
//#define WITH_ISOLINE
 ...
 //   #define BACK_COLOR (float3)(.08f, .16f, .34f) 
    #define BACK_COLOR .3f*(float3)(.8f, .65f, .54f) 
#endif

#define FOCUSBLUR .08f

#define PRECISION_FACTOR 1e-3
#define MIN_DIST_RAYMARCHING .01f
#define MAX_DIST_RAYMARCHING 25.f
...
#define LIGHT_VEC normalize((float3)(-1.2,-.7, -.6) )
...
float kaliFractal(float3 p, const Context* ctx) {
    ...
    return length(p)*pow(Scale, -(float)(NB_ITERATIONS))*.9f;
}
...
float3 renderScene(float3 ro, float3 rd, const Context* ctx, float* dist) {
 ...
 // Color
        col = PALETTE((.02f+sqrt(res.x)));
 // lighting        
        float3 lig = LIGHT_VEC; 
        float3 hal = normalize( lig-rd);

#ifdef WITH_AO
        float occ = calcAO4( pos, nor, ctx);//calcAO( pos, nor, ctx);
#else
        float occ = 1.f;
#endif
	...
	col = (float3)occ*(.5f+.5f*sh);
        ...
        fre *= fre;
        float spe = pow(clamp( dot( ref, lig ), 0.f, 1.f ),36.f);
