...
#define LIGHT_VEC normalize((float3)(.2,.7, .6) )
...
float pseudoKleinianSlider(float3 p, const Context* ctx) {
	float4 q = (float4)(p, 1);
    float4 c = q;
        
    for(int i = 0; i < NB_ITERATION; i++) {
        q.xyz = 2.f*clamp(q.xyz,ctx->mins.xyz, ctx->maxs.xyz) - q.xyz;
        q *=  clamp(1.f/dot(q.xyz, q.xyz), 1.f, ctx->mins.w);
        q = 3.f*q - c;
	}
  //   float rxy=length(q.xy);
  // return .5f*max(-rxy+ctx->maxs.w, (rxy*q.z) / length(q.xyz))/q.w;
    return .5f*length(q.xyz)/q.w;
}
/*
float pseudoKleinianSlider(float3 p, const Context* ctx) {//knighty's pseudo kleinian
  ...
  //  float d0 = max(-rxy+ctx->maxs.w, (rxy*p.z) / length(p))/scale;


p /= scale;
p *= ctx->maxs.w*10.f;

float d1 = sdSponge(p)/(ctx->maxs.w*10.f);

   return .1f*d1; //max((-rxy+ctx->maxs.w)/scale, d1);
   //return .2f*sdBox(p, (float3)(.005f*scale))/scale;
  //  float rxy=length(p.xy);
  //  return .25f*max((d - ctx->maxs.w, d1);
}
*/
