...
#define DE_RAY mandelBulb
//pseudoKleinianSlider
//mandelBulb
//sierpinski
#define DE_COLOR color
//sierpinskiColor
...
{
  return length(max(fabs(p)-b,0.f));
}

float3 color(float3 p, const Context* ctx) {
...
//R(p.xy,.5f); 
   ...
   return udBox(p+.5f, (float3)(1.5f))/r;//(sqrt(dm)-.2f)/r;
}
