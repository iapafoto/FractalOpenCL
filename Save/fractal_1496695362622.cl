...
float3 pseudoKleinianSliderColor(float3 p, const Context* ctx) {//knighty's pseudo kleinian
    ...
    return (float3)(0.f, .15f+.75f*sqrt(orb), orb);
}
...
float3 renderScene( float3 ro, float3 rd, const Context* ctx, float* dist, float iFrame) {
		...
		obj0.colour = (float3)(1.f);
		obj0.type = DIFF;
Material objRef;
		objRef.emission = (float3)(1.f)*.1f;
		objRef.colour = (float3)(1.f,.5f,.5f);
		objRef.type = REFR;
...
obj.emission = obj.colour*1000.f; //(float3)(1.0f,1.0f,1.0f)*50.f;
} else
					if (res.z<.5f || res.x>=MAX_DIST_RAYMARCHING || length(pt.xy)<1.f) {
						
					obj = objLight; 
if (res.z>.4f) obj.emission *= .001f;
}else
					{
						obj = objRef;
