...
#define MAX_DIST_RAYMARCHING 250.f

#define MIN_DIST_SHADOW 10.f*PRECISION_FACTOR
#define MAX_DIST_SHADOW 1.f
#define PRECISION_FACTOR_SHADOW 1e-3

#define MIN_DIST_AO 2.f*PRECISION_FACTOR
#define MAX_DIST_AO 50.f*PRECISION_FACTOR
...
float kaliFractal(float3 p, const Context* ctx) {
    ...
    const float3 Julia = (float3)(-1.2,-1.95,-.6);//ctx->mins.xyz;
    for (int i=0; i<NB_ITERATIONS; i++) {
        p.xy = fabs(p.xy);
        p = p*Scale + Julia;
        p = mvmul3(ctx->rot, p);
